...
export const updateToStaticValue = async (table: string, rowConfig: RowConfig) => {
    ...
    if (sets.length > 0) {
        const query = `UPDATE ?? SET ${sets.join(', ')}`;
...
export const updateToFakerValue = async (table: string, rowConfig: RowConfig) => {
    ...
    rowConfig.fakerValue.forEach(({columnName, value}) => {
    ...
    const {rows: countRows} = await client.execute(`SELECT COUNT(??) as count FROM ??`, [columnId, table]);
    ...
    for (let j = 0; j < 10; j++) {
        ...
        await client.execute(`INSERT INTO ANONYMIZER_JOIN_TABLE (${columnPlaceholders}) VALUES ${v}`, [...schemaWithoutId, ...[...queue].flat(1)]);
        queue.clear();
    }
    pb.render(10_000)
